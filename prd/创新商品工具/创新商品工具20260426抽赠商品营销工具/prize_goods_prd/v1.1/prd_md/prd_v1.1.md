...
- 页面模块：
  - 基本配置（上半区）
    ...
    - 玩法绑定：选择活动玩法，单选
        - 默认项目：抽吧唧活动
        - 抽福袋活动（置灰）
    - 虚拟商品ID：单个虚拟商品ID，支持输入ID下拉搜索选择框添加
    - 费用策略：选择活动使用的策略，单选
        - **费用策略类型**：
  ...
  - 商品信息（下半区）
    - 商品列表：支持输入商品ID下拉搜索选择框添加，展示已添加的商品ID、SKU、商品名称、成本价、活动库存、告警库存、库存同步方式、安全库存、告警状态等信息
...
| 字段 | 校验规则 | 错误提示 |
| --- | --- | --- |
| 费用策略类型 | 必填，枚举值校验 | 请选择费用策略类型 |
| 费用计算基准 | 必填，枚举值校验 | 请选择费用计算基准 |
| 费用承担方 | 必填，枚举值校验 | 请选择费用承担方 |
| 分摊比例 | 当费用承担方为 `split` 时必填，范围 0-100 整数 | 分摊比例必须为 0-100 的整数 |
| 告警阈值 | 当告警开关开启时必填，根据阈值类型校验 | 请输入有效的告警阈值 |
| 活动库存 | 必填，整数且 ≥0 | 活动库存必须为非负整数 |
| 告警库存 | 告警开启时必填，整数且 ≥0，建议 ≤ 活动库存 | 告警库存必须为非负整数且不超过活动库存 |
| 安全库存 | 实时同步模式下可选，整数且 ≥0 | 安全库存必须为非负整数 |

### 7.2 边界场景

- **费用承担方切换**：当费用承担方从 `split` 切换为其他值时，清空分摊比例字段
- **告警开关关闭**：告警开关关闭时，隐藏告警阈值类型和告警阈值字段，已填值保留但不校验
- **库存同步方式切换**：从 `realtime` 切换为 `manual` 时，安全库存字段隐藏但保留值
- **批量导入字段缺失**：导入模板中缺失可选字段时，使用默认值填充；缺失必填字段时，整行导入失败并提示
...
## 9. 未来演进规划
 ...
 - v1.2 支持多玩法类型扩展（抽福袋、抽奖等）
 - v1.2 告警功能实时通知接入企业微信/钉钉
