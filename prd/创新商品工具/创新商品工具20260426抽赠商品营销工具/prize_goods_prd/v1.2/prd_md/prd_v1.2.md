# 营销工具-新增抽赠商品 PRD v1.2

## 1. 项目信息与版本记录
- **产品名称**：营销工具 - 新增抽赠商品
- **版本**：v1.2
- **负责人**：林长宇
- **创建时间**：2026-04-26
- **更新时间**：2026-05-13

### 版本迭代记录
| 版本 | 日期 | 变更项 | 负责人 |
| --- | --- | --- | --- |
| v1.0 | 2026-04-26 | 初始版本，完成需求确认与设计方案初稿 | 林长宇 |
| v1.1 | 2026-05-13 | 调整配置字段细节：优化费用策略字段结构、细化库存配置字段、补充告警规则字段说明 | 林长宇 |
| v1.2 | 2026-05-19 | 展示虚拟商品名称和销售价、实物商品抽赠概率 | 林长宇 |

## 2. 需求背景与目标
### 2.1 背景与痛点
当前营销活动中，抽赠玩法的实物奖品管理流程不统一：
- 不同活动类型（如抽吧唧、抽奖）在奖品配置、审核和发放环节上存在分散处理
- 部分活动通过优惠券作为奖品，导致用户交互流程变长、奖品体验不一致
- 缺少统一的奖品商品池管理和营销费用策略配置入口，导致运营与财务协同成本高

### 2.2 目标
通过新增抽赠商品管理能力，建立统一的奖品商品池与营销费用策略配置体系，使不同玩法类型共享一套奖品审批与费用配置流程。

### 2.3 业务目标
- 支持运营快速配置抽赠玩法的奖品商品池
- 支持财务针对奖品成本价和抽赠方式进行审核
- 统一不同玩法的奖品管理与营销费用参数配置
- 让实体商品奖品以在售商品形态规范管理，避免优惠券奖品的流程混乱


## 3. 用户与场景
### 3.1 核心用户
- **运营人员**：负责活动玩法绑定、奖品池配置和商品导入
- **财务人员**：负责审核奖品成本价与抽赠方式，决定后续营销费用策略
- **活动产品经理**：负责活动玩法联动与奖品暂存逻辑

### 3.2 典型场景
1. 运营在后台创建活动玩法时，为玩法绑定虚拟商品ID并配置对应奖品商品池
2. 运营通过输入商品ID或批量导入/Excel导入方式，将奖品商品导入商品库
3. 导入后，运营需要调整商品 SKU 的活动库存和告警库存，并选择是否启用告警
4. 财务审核奖品成本价与抽赠方式，确定项目的营销费用策略参数
5. 活动类型统一使用本产品配置的奖品商品池与费用策略参数，实现通用流程管理

### 3.3 系统架构和流程

本节说明运营、财务、用户三类角色与三个产品模块之间的关系，明确“抽赠商品管理”“抽吧唧活动管理”“抽吧唧活动页面（另行规划）”三者的职责与交互边界。

#### 3.3.1 角色与模块关系表

| 角色 | 抽赠商品管理 | 抽吧唧活动管理 | 抽吧唧活动页面（另行规划） |
| --- | --- | --- | --- |
| 运营 | 创建/编辑抽赠商品配置，绑定玩法与虚拟商品ID，导入奖品商品池，配置库存与告警，选择费用策略 | 查看活动列表的玩法绑定与费用策略，确认活动状态和关联配置 | 无直接访问；使用由活动页面提供的玩法入口和奖品展示，活动内容由另行规划的页面团队设计 |
| 财务 | 审核奖品成本价与抽赠方式，确认费用策略参数，审核结果返回运营 | 查看活动级别的关联策略和审核结果，评估活动成本与可上线状态 | 不直接参与页面交互，但其审核结果决定活动页面是否可正常开放及费用计算参数 |
| 用户 | 无直接访问；其奖品体验依赖抽赠商品池与费用策略配置 | 无直接访问；其玩法行为依赖抽吧唧活动管理传递的玩法设置 | 访问抽吧唧活动页面参与抽奖，展示玩法与奖品说明；抽奖结果与奖品发放逻辑由活动玩法系统处理 |

#### 3.3.2 产品模块职责与关系

| 产品模块 | 主要职责 | 对其它模块的依赖 |
| --- | --- | --- |
| 抽赠商品管理 | 统一抽赠奖品商品池配置、商品导入、库存与告警配置、费用策略参数设置 | 为抽吧唧活动管理提供玩法绑定、虚拟商品ID、费用策略模板和奖品池数据；为抽吧唧活动页面提供后台奖品数据和费用参数依据 |
| 抽吧唧活动管理 | 管理抽吧唧活动列表、玩法关联、状态与活动级参数展示 | 读取抽赠商品管理的配置，展示活动当前的玩法绑定与费用策略；输出活动配置给抽吧唧活动页面 |
| 抽吧唧活动页面（另行规划） | 提供用户端抽吧唧玩法入口、抽奖交互和结果展示 | 依赖抽吧唧活动管理传递的玩法配置及抽赠商品管理的奖品池与费用策略数据 |

#### 3.3.3 关系流程图

```mermaid
flowchart LR
    subgraph 运营端
      A[抽赠商品管理]
      B[抽吧唧活动管理]
    end
    subgraph 用户端
      C[抽吧唧活动页面]
    end

    A -->|提供玩法绑定、虚拟商品ID与奖品池| B
    A -->|提供奖品数据与费用策略| C
    B -->|提供活动配置与发布状态| C

    subgraph 角色
      O(运营)
      F(财务)
      U(用户)
    end

    O --> A
    O --> B
    F --> A
    U --> C

    classDef product fill:#f2f9ff,stroke:#4c7eff,stroke-width:1px;
    class A,B,C product;
```

## 4. 产品范围
### 4.1 运营后台

#### 4.1.1 运营后台-营销管理-抽赠商品管理模块

- 基本设置
    - 玩法绑定与玩法虚拟商品ID设置
    - 营销费用策略参数配置（单选项，默认项目：0元赠送，按销售价计算营销费用）
        - **费用策略类型**（`fee_strategy_type`）：枚举值，`free_sale_price`（0元赠送-按销售价计算）、`virtual_order_merge`（关联虚拟订单合并计算），默认 `free_sale_price`
- 奖品商品池管理
    - 奖品商品池配置与商品导入管理
    - 批量导入/Excel导入操作入口展示与导入后校验
    - 奖品商品 SKU 的活动库存
        - **活动库存**（`activity_stock`）：数值型，必填，范围 ≥0


#### 4.1.2 运营后台-营销管理-抽吧唧活动管理模块

- 抽吧唧活动管理
    - 仅展示活动列表中相关参数：
        - 玩法绑定的虚拟商品ID
        - 费用策略参数（默认项目：0元赠送，按销售价计算营销费用）

### 4.2 产品确认事项

### 4.2.1 本次产品确认事项

- 本期仅支持策略一（0元赠送-按销售价计算营销费用）用于抽吧唧活动
- 玩法维度一套虚拟商品ID对应一组奖品池，保障配置关联清晰
- 费用策略仅参数配置，具体费用计算逻辑可由后续业务线（寄存柜下单）使用该参数实现
- 抽吧唧活动同时只能启用一个同一虚拟商品ID
- 抽赠商品的后台界面暂时先集成在抽吧唧配置后台，架构不变
  - 1、二次编辑时，涉及实物奖品新增、关联概率等财务风险调整，必须下线活动后编辑，重新走财务审核流程，财务复审后，方可重新上线；
  - 2、其他编辑，按高老师的prd，走活动审核流程；
- 订单需要关联营销活动类型和ID方便统计分析营销费用，此次不单独计算营销费用，仅记录关联关系
- 虚拟商品订单关联实物商品订单采取最后归因关联算法，同一商品在未邮寄前仅关联记录最后一个虚拟商品订单号，并在实物订单的otherinfo中记录虚拟商品订单号
- 商品类型目前无法限制，需要在活动配置后台限制，虚拟商品ID仅限小芒自营、一件代发，虚拟商品ID与实物商品ID必须需要同一店铺，后台保存时做一下校验并给一个合适的错误提示
  - “目前仅支持小芒自营、一件代发商品”
  - “虚拟商品ID与实物商品ID必须需要同一店铺”

### 4.2.2 本次产品确认不处理的事项

- 告警功能暂不处理
- 活动玩法本身的抽赠方式分类设计
- 寄存柜页面展示与用户下单发货流程
- 奖品暂存过期后的删除或清理逻辑
- 活动级别的最终发货与售后流程

## 5. 功能清单
### 5.1 页面形态与操作入口
#### 5.1.1 运营后台-营销管理-抽赠商品管理模块
#### 5.1.1.1 抽赠商品管理列表页
- 入口位置：运营后台 > 营销管理 > 抽赠商品管理
- 页面模块：
    - 抽赠商品列表展示：活动名称/玩法/费用策略/归属部门/归属营销活动/提报人/活动时间/活动审核/商品审核/操作等关键信息
      - 活动时间（上面是开始时间，下面是结束时间，时分秒级展示）
      - 商品审核展示已上线商品数量，待审核商品数量
    - 操作按钮：
      - 单独的新增按钮
      - 列表中的编辑、递交审核/撤回审核/审核/启用/停用、复制
    - 筛选条件：项目/玩法/状态

#### 5.1.1.2 抽赠商品管理添加弹层
- 入口位置：运营后台 > 营销管理 > 抽赠商品管理 - 新增
- 页面模块：
  - 基本配置（上半区）
    - 活动名称：输入活动名称
    - 玩法绑定：选择活动玩法，单选
        - 默认项目：抽吧唧活动
        - 抽福袋活动（置灰）
    - 虚拟商品ID：单个虚拟商品ID，支持输入ID下拉搜索选择框添加，填写虚拟商品ID后，右侧展示商品名称和销售价
    - 费用策略：选择活动使用的策略，单选
        - **费用策略类型**：
            - 0元赠送-按销售价计算营销费用 （默认项目）
            - 关联虚拟订单合并计算营销费用（置灰）
    - 归属部门：选择活动归属部门，单选
    - 归属营销活动：选择活动归属营销活动，单选
    - 活动起止时间：选择活动开始时间和结束时间，到时分秒级
  - 商品信息（下半区）
    - 商品列表：支持输入商品ID下拉搜索选择框添加，展示已添加的商品ID、SKU、商品名称、成本价、活动库存、概率、审核状态等信息
      - 在审核界面的实物商品列表中添加概率字段，展示"绑定xxxx，抽中概率0.01%"，可多行展示
      - 支持商品维度的审核状态展示和部分通过审核功能
    - 批量操作：批量设置活动库存和告警库存，批量启用/关闭告警，批量设置库存同步方式
    - 导入操作：支持批量导入/Excel导入方式添加商品（本方案中仅示意）
      - 导入模板字段：商品ID、SKU、活动库存、告警库存、库存同步方式（manual/realtime）、安全库存
  - 保存与取消按钮
- 目标：支持运营完成玩法绑定与项目级费用、告警的基础配置

#### 5.1.2 运营后台-营销管理-抽吧唧活动管理模块

#### 5.1.2.1 抽吧唧活动列表页
- 入口位置：运营后台 > 营销管理 > 抽吧唧活动
- 页面模块：
    - 活动列表展示：活动名称/虚拟商品ID/活动状态/开始时间/结束时间/操作等关键信息展示
    - 操作按钮：编辑、启用/停用
    - 筛选条件：活动名称/虚拟商品ID/状态
- 目标：支持运营查看玩法活动列表


## 6. 业务流程图
### 6.1 奖品商品池配置流程
1. 运营进入【抽赠商品】新增或编辑活动配置
2. 选择玩法类型，绑定对应的虚拟商品ID，选择营销费用计算策略
3. 选择商品ID导入或批量导入/Excel导入
4. 运营在【活动配置（抽吧唧）】配置实物商品的概率
5. 运营在【活动配置（抽吧唧）】保存后递交审核
6. 财务在【抽赠商品】审核待审核奖品列表，通过/拒绝审核，拒绝审核后退回运营修改
7. 运营在【活动配置（抽吧唧）】启用活动


## 7. 异常与边界处理

无


## 8. 数据追踪与埋点

无

## 9. 未来演进规划

 - v2.0 支持多玩法类型扩展（抽福袋）

## 10. 附件

无
