...
flowchart TD
    ...
    F --> G[发布众筹活动]
    G --> H{活动是否开始}
    H -->|未开始| I[App展示开始前状态<br/>按钮：待开启]
    H -->|已开始| J[App端展示众筹信息]
    J --> K[用户浏览商品]
    K --> L{活动是否截止}
    L -->|未截止| M{是否达标}
    M -->|未达标| N[继续进行中<br/>按钮：去支持]
    M -->|已达标| O[达标后继续售卖<br/>按钮：去支持]
    L -->|已截止| P{是否达标}
    P -->|达标| Q[众筹成功<br/>✅成功标识<br/>按钮：已成功]
    P -->|未达标| R[众筹失败<br/>❌失败标识<br/>按钮：已结束]
    Q --> S[订单正常流转]
    R --> T[原路退款，通知用户]
```
...
| 异常场景 | 处理逻辑 |
|----------|----------|
| 进度超额完成 | 进度条按100%显示，文案显示具体件数/金额，大额数字缩略显示 |
| 数据位数过长 | 缩略显示（如"100万"） |
| 活动开始前点击按钮 | 按钮不可点击，提示"活动还未开始" |
...
| `crowd_page_view` | 用户浏览众筹商品详情页 | product_id, current_progress, target, status |
| `crowd_join_click` | 用户点击"去支持" | product_id, user_id, status |
| `crowd_payment_success` | 用户众筹支付成功 | order_id, amount, product_id |
| `crowd_status_change` | 活动状态变更（成功/失败/继续售卖） | activity_id, status, final_progress |
...
| 附件名称 | 说明 |
|----------|------|
| app_prototype_v1.1.html | App端原型 - 包含5个阶段的众筹信息展示 |
| admin_prototype_v1.0.html | 商家后台原型 - 保持v1.0版本 |
